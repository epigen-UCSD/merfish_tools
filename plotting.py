...
def drift_histogram(mfx: 'MerfishExperiment') -> None:
    #remove outliers
...
def exact_vs_corrected(mfx: 'MerfishExperiment') -> None:
    ...
    plt.figure()
    ax = plt.bar([0, 1], [exact, corrected])
...
def per_bit_error_bar(mfx: 'MerfishExperiment') -> None:
    ...
    ax = sns.catplot(x="Hybridization round", y="Error rate", row="Error type", data=data, kind="bar", height=2, aspect=3,
        capsize=0.2, hue='Color', order=sorted(data["Hybridization round"].unique()), hue_order=mfx.barcode_colors,sharex=False, sharey=False)
...
def per_bit_error_line(mfx: 'MerfishExperiment') -> None:
    ...
    ax = sns.catplot(x="Hybridization round", y="Error rate", row="Error type", data=data, kind="point", height=2, aspect=3, ci=None,
        capsize=0.2, hue='Color', order=sorted(data["Hybridization round"].unique()), hue_order=mfx.barcode_colors, sharey=False, sharex=False)
    for axs in ax.axes.flat:
        axs.yaxis.set_major_formatter(pctformatter)
...
def per_hyb_error(mfx: 'MerfishExperiment') -> None:
    ...
    g.map_dataframe(sns.barplot, x="Hybridization round", y="Error rate", ci=None)# capsize=0.2)
    g.map_dataframe(sns.pointplot, x="Hybridization round", y="Error rate", color='#444444', markers='.',capsize=0.2)
...
def per_color_error(mfx: 'MerfishExperiment') -> None:
    ...
    ax = sns.catplot(x="Color", y="Error rate", row="Error type", data=data, kind="bar", height=2, aspect=3,
        capsize=0.2, order=mfx.barcode_colors, sharey=False, sharex=False)
    for axs in ax.axes.flat:
        axs.yaxis.set_major_formatter(pctformatter)
