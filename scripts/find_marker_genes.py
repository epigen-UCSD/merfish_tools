...
Currently, the way this information is loaded into the script is hard-coded
for a specific dataset. In the future, this should be made more general.
"""
import math
import pandas as pd
import scanpy as sc
...
#####
# Do some standard filtering and normalization on the counts table
#####
cdata = sc.read_text('/storage/RNA_MERFISH/Reference_Data/GSE97930_FrontalCortex_snDrop-seq_UMI_Count_Matrix_08-01-2017.txt')
cdata = cdata.T
cdata.obs['cluster'] = [cell.split('_')[0] for cell in cdata.obs.index]
cdata.obs['group'] = [cell.split('_')[1] for cell in cdata.obs.index]

#Remove cells with low number of genes detected
sc.pp.filter_cells(cdata, min_genes=200)
#Remove genes detected in a low number of cells
sc.pp.filter_genes(cdata, min_cells=3)

#Normalize counts to total counts
...
#Regress out the total counts
sc.pp.regress_out(cdata, 'total_counts')

#Scale counts to unit variance
sc.pp.scale(cdata, max_value=10)

#Saved normalized counts
cdata.write('/storage/RNA_MERFISH/Reference_Data/GSE97930_FrontalCortex.h5ad')

#####
# Run NSForest
#####
dataFull = cdata.to_df()
dataFull.columns = [x.translate(str.maketrans({'.':'_', '-':'_', '@':'_'})) for x in dataFull.columns]
dataFull['Clusters'] = cdata.obs['cluster']
...
for k, v in genes.items():
    for i, g in enumerate(v, start=1):
        print('\n'.join(','.join([k,str(i),a]) for a in g))
