...
class Stats:
    ...
    def __init__(self, mfx):
        ...
        self.functions = {
            ...
            "Unfiltered barcodes per FOV": lambda stats: stats[
                "Unfiltered barcode count"
            ]
            / stats["FOVs"],
            "Filtered barcodes per FOV": lambda stats: stats["Filtered barcode count"]
            / stats["FOVs"],
            "% barcodes kept": lambda stats: stats["Filtered barcode count"]
            / stats["Unfiltered barcode count"],
            "Exact barcode count": lambda stats: len(
                self.mfx.unassigned_barcodes[
                    self.mfx.unassigned_barcodes["error_type"] == 0
                ]
            ),
            "Corrected barcode count": lambda stats: stats["Filtered barcode count"]
            - stats["Exact barcode count"],
            "% exact barcodes": lambda stats: stats["Exact barcode count"]
            / stats["Filtered barcode count"],
            "0->1 error rate": lambda stats: len(
                self.mfx.unassigned_barcodes[
                    self.mfx.unassigned_barcodes["error_type"] == 1
                ]
            )
            / stats["Filtered barcode count"],
            "1->0 error rate": lambda stats: len(
                self.mfx.unassigned_barcodes[
                    self.mfx.unassigned_barcodes["error_type"] == -1
                ]
            )
            ...
            "Average per-bit 0->1 error rate": lambda stats: self.per_bit_error[
                self.per_bit_error["Error type"] == "0->1"
            ]["Error rate"].mean(),
            "Average per-bit 1->0 error rate": lambda stats: self.per_bit_error[
                self.per_bit_error["Error type"] == "1->0"
            ]["Error rate"].mean(),
            "Segmented cells": lambda stats: len(np.unique(self.mfx.celldata.index)),
            "Segmented cells per FOV": lambda stats: stats["Segmented cells"]
            / stats["FOVs"],
            "Median cell volume (pixels)": lambda stats: np.median(
                stats.mfx.celldata["volume"]
            ),
            "Barcodes assigned to cells": lambda stats: len(
                self.mfx.assigned_barcodes[
                    self.mfx.assigned_barcodes["status"] == "good"
                ]
            ),
            "% barcodes assigned to cells": lambda stats: stats[
                "Barcodes assigned to cells"
            ]
            / len(
                self.mfx.assigned_barcodes[
                    self.mfx.assigned_barcodes["status"] != "edge"
                ]
            ),
            "Cells with barcodes": lambda stats: len(
                np.unique(
                    self.mfx.assigned_barcodes[
                        self.mfx.assigned_barcodes["status"] == "good"
                    ]["cell_id"]
            ...
            "% cells with barcodes": lambda stats: stats["Cells with barcodes"]
            / stats["Segmented cells"],
            "Median transcripts per cell": lambda stats: np.median(
                self.mfx.single_cell_raw_counts.apply(np.sum, axis=1)
            ),
            "Median genes detected per cell": lambda stats: np.median(
                self.mfx.single_cell_raw_counts.astype(bool).sum(axis=1)
            ),
        ...
        if config.has("reference_counts"):
            for dataset in config.get("reference_counts"):
                self.functions[
                    dataset["name"]
                ] = lambda stats: self.correlation_with_reference(dataset["file"])
    ...
    def error_counts_prefiltered(self) -> pd.DataFrame:
        ...
        for filename in tqdm(
            files, desc="Sampling pre-filtering barcodes", total=float("inf")
        ):
            newdata = process_fov(filename, normcodes, codebook)
            data = pd.concat([data, newdata], ignore_index=True)
            stats = error_stats(data)
            if (
                abs(stats["0->1 error rate"] - zero2one) < 0.001
                and abs(stats["1->0 error rate"] - one2zero) < 0.001
            ):
                stable_count += 1
                print(stable_count)
                if stable_count >= 3:
    ...
    def per_gene_error(self) -> pd.DataFrame:
        ...
        temp = (
            self.mfx.unassigned_barcodes.groupby(["gene", "error_type"])
            .count()
            .reset_index()
        ...
        temp["Total barcodes"] = (
            temp["1->0 errors"] + temp["0->1 errors"] + temp["Exact barcodes"]
        )
    ...
    def per_bit_error(self) -> pd.DataFrame:
        ...
        err_rates = pd.concat(
            get_per_bit_stats(gene, group)
            for gene, group in self.mfx.unassigned_barcodes.groupby("gene")
        )
        err_rates["Color"] = err_rates.apply(
            lambda row: self.mfx.barcode_colors[
                (row["Bit"] - 1) % len(self.mfx.barcode_colors)
            ],
            axis=1,
        )
        err_rates["Hybridization round"] = err_rates.apply(
            lambda row: ((row["Bit"] - 1) // len(self.mfx.barcode_colors)) + 1, axis=1
        )
        return err_rates
    ...
    def per_fov_error(self) -> pd.DataFrame:
        ...
        foverr = (
            self.mfx.unassigned_barcodes.groupby(["fov", "error_type"]).count()["gene"]
            / self.mfx.unassigned_barcodes.groupby("fov").count()["gene"]
        )
        foverr = foverr.reset_index()
        foverr.columns = ["FOV", "Error type", "Error rate"]
        foverr["Error type"] = foverr["Error type"].replace(
    ...
    def reference_gene_counts(self, filename) -> dict:
        if filename not in self._ref_counts:
            refcounts = pd.read_csv(
                "/storage/RNA_MERFISH/Reference_Data/W15_all_genes_norm.csv"
            )
            self._ref_counts[filename] = dict(
                zip(refcounts["geneName"], np.log10(refcounts["counts"]))
            )
